...
class UNetEncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, leaky_relu_slope=0.2):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(5, 5), stride=(2, 2), padding=(2, 2))
        self.bn = nn.BatchNorm2d(num_features=out_channels)
        self.leaky_relu_slope = leaky_relu_slope
        self.act = nn.LeakyReLU(self.leaky_relu_slope)
        self.apply(self.init_weights)

    def init_weights(self, m):
        if isinstance(m, nn.Conv2d):
            nn.init.kaiming_uniform_(m.weight, nonlinearity='leaky_relu', a=self.leaky_relu_slope)
...
class UNetDecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dropout=0.):
        super().__init__()
        self.conv_trans = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=(5, 5), stride=(2, 2),
